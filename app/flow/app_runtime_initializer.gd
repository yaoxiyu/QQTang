...
static func request_initialize(runtime: Node) -> Dictionary:
	...
	runtime._ensure_runtime_contexts()
	runtime._ensure_runtime_config()
	runtime._ensure_front_repositories()
	runtime._ensure_front_local_state()
	runtime._ensure_front_services()
	runtime._ensure_resume_state_store()
	AppNavigationCoordinatorScript.ensure_navigation(runtime)
	_ensure_runtime_components(runtime)
	runtime._ensure_front_use_cases()
	AppNavigationCoordinatorScript.ensure_boot_state(runtime)
	return {"ok": true}


static func _ensure_runtime_components(runtime: Node) -> void:
