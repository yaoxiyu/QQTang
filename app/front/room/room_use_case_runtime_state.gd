...
static func get_current_room_phase(app_runtime: Node) -> String:
	if app_runtime == null or app_runtime.current_room_snapshot == null:
		return ""
	var room_phase := String(app_runtime.current_room_snapshot.room_phase)
	if not room_phase.is_empty():
		return room_phase
	var room_lifecycle_state := String(app_runtime.current_room_snapshot.room_lifecycle_state)
	if not room_lifecycle_state.is_empty():
		return room_lifecycle_state
	return ""
...
static func get_current_queue_phase(app_runtime: Node) -> String:
	if app_runtime == null or app_runtime.current_room_snapshot == null:
		return ""
	return String(app_runtime.current_room_snapshot.queue_phase)


static func can_cancel_current_queue(app_runtime: Node) -> bool:
	if app_runtime == null or app_runtime.current_room_snapshot == null:
		return false
	if bool(app_runtime.current_room_snapshot.can_cancel_queue):
		return true
