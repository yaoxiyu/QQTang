...
func refresh_catalog() -> Dictionary:
	...
	var response = await shop_gateway.fetch_catalog(auth_session_state.access_token, revision)
...
func purchase_offer(offer_id: String, idempotency_key: String = "") -> Dictionary:
	...
	_configure_gateway()
	var response = await shop_gateway.purchase_offer(auth_session_state.access_token, offer_id, key, current_catalog.catalog_revision)
	if not bool(response.get("ok", false)):
...
func _configure_gateway() -> void:
	...
	if shop_gateway != null and shop_gateway.has_method("configure_base_url"):
		shop_gateway.configure_base_url(ServiceUrlBuilderScript.build_account_base_url(front_settings_state.account_service_host, front_settings_state.account_service_port, 18080))
