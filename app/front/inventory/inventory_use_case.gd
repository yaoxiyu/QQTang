...
func refresh_inventory() -> Dictionary:
	...
	_configure_gateway()
	var response = await inventory_gateway.fetch_my_inventory(auth_session_state.access_token)
...
func _configure_gateway() -> void:
	...
	if inventory_gateway != null and inventory_gateway.has_method("configure_base_url"):
		inventory_gateway.configure_base_url(ServiceUrlBuilderScript.build_account_base_url(front_settings_state.account_service_host, front_settings_state.account_service_port, 18080))
