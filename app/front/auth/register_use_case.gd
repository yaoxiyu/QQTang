...
func _fetch_and_apply_profile() -> Dictionary:
	...
	if app_runtime.player_profile_state != null:
		...
		app_runtime.player_profile_state.owned_character_skin_ids = PlayerProfileState._to_string_array(result.get("owned_character_skin_ids", []))
		...
		app_runtime.player_profile_state.owned_bubble_skin_ids = PlayerProfileState._to_string_array(result.get("owned_bubble_skin_ids", []))
