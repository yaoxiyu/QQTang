...
static func success_from_dict(data: Dictionary):
	...
	result.assignment_id = String(data.get("assignment_id", ""))
	result.match_source = String(data.get("match_source", ""))
	result.season_id = String(data.get("season_id", ""))
	result.locked_map_id = String(data.get("locked_map_id", ""))
	result.locked_rule_set_id = String(data.get("locked_rule_set_id", ""))
	result.locked_mode_id = String(data.get("locked_mode_id", ""))
	result.assigned_team_id = int(data.get("assigned_team_id", 0))
	result.expected_member_count = int(data.get("expected_member_count", 0))
	result.auto_ready_on_join = bool(data.get("auto_ready_on_join", false))
