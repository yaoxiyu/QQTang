...
static func fail(p_error_code: String, p_user_message: String) -> RegisterResult:
	var result := RegisterResult.new()
	result.ok = false
...
static func success_from_dict(data: Dictionary) -> RegisterResult:
	var result := RegisterResult.new()
