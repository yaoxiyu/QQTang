...
func refresh_wallet() -> Dictionary:
	...
	_configure_gateway()
	var response = wallet_gateway.fetch_my_wallet(auth_session_state.access_token)
...
func _configure_gateway() -> void:
	...
	if wallet_gateway != null and wallet_gateway.has_method("configure_base_url"):
		wallet_gateway.configure_base_url("http://%s:%d" % [front_settings_state.account_service_host, front_settings_state.account_service_port])
