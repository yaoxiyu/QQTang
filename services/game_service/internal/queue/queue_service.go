	...
	for partyIndex, party := range selected {
		...
		for _, queuedMember := range members {
			...
			member := storage.AssignmentMember{
				...
				CharacterID:     queuedMember.CharacterID,
				CharacterSkinID: queuedMember.CharacterSkinID,
				BubbleStyleID:   queuedMember.BubbleStyleID,
				BubbleSkinID:    queuedMember.BubbleSkinID,
				JoinState:       "assigned",
				ResultState:     "",
				CreatedAt:       now,
				UpdatedAt:       now,
