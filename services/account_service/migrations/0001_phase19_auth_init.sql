...
CREATE UNIQUE INDEX uq_accounts_login_name
    ON accounts (login_name);

CREATE INDEX idx_accounts_status
    ON accounts (status);

CREATE INDEX idx_accounts_last_login_at
...
CREATE TABLE player_profiles (
    ...
    default_character_id        VARCHAR(64) NOT NULL DEFAULT 'character_default',
    default_character_skin_id   VARCHAR(64) NOT NULL DEFAULT 'skin_default',
    default_bubble_style_id     VARCHAR(64) NOT NULL DEFAULT 'bubble_style_default',
    default_bubble_skin_id      VARCHAR(64) NOT NULL DEFAULT 'bubble_skin_default',
...
CREATE INDEX idx_player_profiles_nickname
    ON player_profiles (nickname);

CREATE INDEX idx_player_profiles_updated_at
    ON player_profiles (updated_at);


CREATE TABLE player_owned_assets (
    account_id           VARCHAR(64) NOT NULL,
...
CREATE INDEX idx_player_owned_assets_account_id
    ON player_owned_assets (account_id);

CREATE INDEX idx_player_owned_assets_profile_id
    ON player_owned_assets (profile_id);

CREATE INDEX idx_player_owned_assets_asset_lookup
    ON player_owned_assets (asset_type, asset_id);

CREATE INDEX idx_player_owned_assets_acquired_at
...
CREATE INDEX idx_account_sessions_device_session_id
    ON account_sessions (device_session_id);

CREATE INDEX idx_account_sessions_refresh_expire_at
    ON account_sessions (refresh_expire_at);

CREATE INDEX idx_account_sessions_last_seen_at
    ON account_sessions (last_seen_at);

CREATE INDEX idx_account_sessions_active_account
    ON account_sessions (account_id, refresh_expire_at DESC)
    WHERE revoked_at IS NULL;


CREATE TABLE room_entry_tickets (
...
CREATE INDEX idx_room_entry_tickets_account_id
    ON room_entry_tickets (account_id);

CREATE INDEX idx_room_entry_tickets_profile_id
    ON room_entry_tickets (profile_id);

CREATE INDEX idx_room_entry_tickets_room_id
    ON room_entry_tickets (room_id);

CREATE INDEX idx_room_entry_tickets_expire_at
    ON room_entry_tickets (expire_at);

CREATE INDEX idx_room_entry_tickets_purpose
    ON room_entry_tickets (purpose);

CREATE INDEX idx_room_entry_tickets_active_lookup
