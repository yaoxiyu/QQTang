...
func mapTicketError(err error) (int, string) {
	switch {
	case errors.Is(err, ticket.ErrPurposeInvalid), errors.Is(err, ticket.ErrTargetInvalid), errors.Is(err, ticket.ErrRequestedMatchInvalid), errors.Is(err, ticket.ErrMatchRoomAssignmentRequired):
