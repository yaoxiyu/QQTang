...
> Archival note: this file is a historical Current snapshot and is not current implementation truth. Current source structure and responsibilities are defined by `docs/current_source_of_truth.md`.
> Purpose: freeze the current Current-closeout baseline before Current implementation.
> Source date: 2026-04-02
> Source of truth priority: `docs/current_source_of_truth.md`
> Archival note: this file is a **Current historical snapshot**. For current implementation truth after Current resource formalization, prefer `docs/current_source_of_truth.md` and the latest Current documents.
...
- Current should extend startup / finish / return-room stability, not rebuild battle entry.
...
- It still exists as a runnable transport shell and remains a likely source of future misuse if not further downgraded in Current.
...
- `test_square` is still present in formal catalog output and is a known Current cleanup target.
...
Current Current formalized resource paths:
...
- Historical note:
  - `CharacterResource` was a temporary compatibility layer during the early Current migration
...
- Runtime currently tolerates invalid ids by silently resolving to default, which Current may need to tighten earlier in room validation.
...
## 6. Immediate Current risks confirmed by baseline
...
Current therefore should proceed as:
