...
Native kernels are already the default path. Current adds a formal schema so future native work can move from Variant/Dictionary parsing toward packed stride access.
...
Current adds metrics and limits, not delta snapshots or custom allocators:
