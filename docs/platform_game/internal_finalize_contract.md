...
- Current: After finalize commits, `game_service` notifies `room_service` that the source room can transition from `in_battle_frozen` to `awaiting_return`. This enables the post-battle return-to-source-room flow.
