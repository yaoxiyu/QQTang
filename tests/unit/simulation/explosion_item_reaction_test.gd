...
func _ready() -> void:
	...
	if ok:
		print("explosion_item_reaction_test: PASS")
...
func _test_destroy_profile_removes_item() -> bool:
	...
	var ok := true
	ok = TestAssert.is_true(item != null and not item.alive, "destroy profile should despawn hit item", prefix) and ok
	ok = TestAssert.is_true(not world.state.items.active_ids.has(item_id), "destroy profile should remove item from active list", prefix) and ok
	world.dispose()
	return ok
...
func _test_transform_profile_updates_item_type_in_place() -> bool:
	...
	var prefix := "explosion_item_reaction_test"
	var ok := true
	ok = TestAssert.is_true(item != null and item.alive, "transform profile should keep item alive", prefix) and ok
	ok = TestAssert.is_true(item != null and item.item_type == 3, "transform profile should update item type to speed", prefix) and ok
	ok = TestAssert.is_true(world.state.items.active_ids.has(item_id), "transform profile should keep same item id active", prefix) and ok
	world.dispose()
	return ok
...
func _test_ignore_profile_keeps_item_alive() -> bool:
	...
	var prefix := "explosion_item_reaction_test"
	var ok := true
	ok = TestAssert.is_true(item != null and item.alive, "ignore profile should keep item alive", prefix) and ok
	ok = TestAssert.is_true(item != null and item.item_type == 1, "ignore profile should keep original item type", prefix) and ok
	world.dispose()
	return ok
