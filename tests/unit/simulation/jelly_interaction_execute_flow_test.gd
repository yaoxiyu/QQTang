...
func _test_enemy_touch_executes_trapped_player_in_same_tick() -> bool:
	...
	var ok := true
	ok = qqt_check(victim_after != null and not victim_after.alive, "enemy touch should finalize trapped victim death", prefix) and ok
	ok = qqt_check(victim_after != null and victim_after.life_state == PlayerState.LifeState.DEAD, "classic victim should enter DEAD", prefix) and ok
	ok = qqt_check(not world.state.players.active_ids.has(victim.entity_id), "dead victim should leave active ids", prefix) and ok
	ok = qqt_check(_has_event(result["events"], SimEvent.EventType.PLAYER_KILLED), "PLAYER_KILLED should be emitted in same tick", prefix) and ok
	world.dispose()
	return ok
...
func _test_enemy_touch_executes_adjacent_blocked_jelly_in_same_tick() -> bool:
	...
	var ok := true
	ok = qqt_check(victim_after != null and not victim_after.alive, "adjacent blocked enemy touch should finalize trapped victim death", prefix) and ok
	ok = qqt_check(victim_after != null and victim_after.life_state == PlayerState.LifeState.DEAD, "adjacent victim should enter DEAD", prefix) and ok
	ok = qqt_check(_has_event(result["events"], SimEvent.EventType.PLAYER_KILLED), "adjacent touch should emit PLAYER_KILLED in same tick", prefix) and ok
	world.dispose()
	return ok
...
func _test_score_mode_enemy_touch_scores_and_starts_respawn() -> bool:
	...
	var ok := true
	ok = qqt_check(victim_after != null and not victim_after.alive, "score victim should be inactive while respawning", prefix) and ok
	ok = qqt_check(victim_after != null and victim_after.life_state == PlayerState.LifeState.REVIVING, "score victim should enter REVIVING", prefix) and ok
	ok = qqt_check(attacker_after != null and attacker_after.score == 1, "finisher personal score should increment", prefix) and ok
	ok = qqt_check(int(world.state.mode.team_scores.get(attacker.team_id, 0)) == 1, "finisher team score should increment", prefix) and ok
	ok = qqt_check(_has_event(result["events"], SimEvent.EventType.PLAYER_KILLED), "PLAYER_KILLED should be emitted before respawn countdown", prefix) and ok
	ok = qqt_check(_has_player_pose(world, victim.entity_id, "defeat"), "score victim should stay visible in defeat pose while reviving", prefix) and ok
	world.dispose()
	return ok
...
func _test_trapped_player_auto_executes_after_timeout() -> bool:
	...
	var ok := true
	ok = qqt_check(victim_after != null and victim_after.life_state == PlayerState.LifeState.REVIVING, "timed-out jelly should enter REVIVING in score mode", prefix) and ok
	ok = qqt_check(victim_after != null and victim_after.trapped_timeout_ticks == 0, "timed-out jelly should consume trapped timeout", prefix) and ok
	ok = qqt_check(_has_event(result["events"], SimEvent.EventType.PLAYER_KILLED), "timed-out jelly should emit PLAYER_KILLED", prefix) and ok
	world.dispose()
	return ok
...
func _test_trap_and_death_preserve_subcell_offset() -> bool:
	...
	ok = qqt_check(trapped != null and trapped.life_state == PlayerState.LifeState.TRAPPED, "victim should enter TRAPPED", prefix) and ok
	ok = qqt_check(trapped != null and trapped.offset_x == 300 and trapped.offset_y == -200, "TRAPPED should preserve subcell offset", prefix) and ok
	...
	ok = qqt_check(dead != null and dead.life_state == PlayerState.LifeState.DEAD, "victim should enter DEAD", prefix) and ok
	ok = qqt_check(dead != null and dead.offset_x == 300 and dead.offset_y == -200, "DEAD should preserve subcell offset", prefix) and ok
	world.dispose()
	return ok
...
func _test_respawn_invincibility_expires_before_next_explosion() -> bool:
	...
	var ok := true
	ok = qqt_check(hit_again != null and hit_again.life_state == PlayerState.LifeState.TRAPPED, "revived player should be hittable after invincibility expires", prefix) and ok
	world.dispose()
	return ok
...
func _test_dead_player_stays_visible_for_death_display_window() -> bool:
	...
	var ok := true
	ok = qqt_check(victim_after != null and victim_after.death_display_ticks > 0, "classic victim should keep death display ticks", prefix) and ok
	ok = qqt_check(_has_player_pose(world, victim.entity_id, "defeat"), "classic victim should stay visible in defeat pose", prefix) and ok

	for _i in range(30):
		world.step()

	var victim_late := world.state.players.get_player(victim.entity_id)
	ok = qqt_check(victim_late != null and victim_late.death_display_ticks == 0, "death display ticks should expire", prefix) and ok
	ok = qqt_check(not _has_player_view(world, victim.entity_id), "classic victim should disappear after display window outside settlement", prefix) and ok
	world.dispose()
	return ok
...
func _test_settlement_overrides_dead_display_pose() -> bool:
	...
	var ok := true
	ok = qqt_check(world.state.match_state.phase == MatchState.Phase.ENDED, "classic kill should end two-team match", prefix) and ok
	ok = qqt_check(_has_player_pose(world, attacker.entity_id, "victory"), "winner should switch to victory pose in settlement", prefix) and ok
	ok = qqt_check(_has_player_pose(world, victim.entity_id, "defeat"), "loser should keep defeat pose in settlement", prefix) and ok
	world.dispose()
	return ok
