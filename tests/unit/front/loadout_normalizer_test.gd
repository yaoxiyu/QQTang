...
func _ready() -> void:
	...
	if ok:
		print("loadout_normalizer_test: PASS")
...
func _test_resolve_from_owned_profile_defaults() -> bool:
	...
	var ok := true
	ok = TestAssert.is_true(result.character_id == "char_huoying", "valid character should be preserved", prefix) and ok
	ok = TestAssert.is_true(result.character_skin_id == "skin_gold", "valid character skin should be preserved", prefix) and ok
	ok = TestAssert.is_true(result.bubble_style_id == "bubble_round", "valid bubble style should be preserved", prefix) and ok
	ok = TestAssert.is_true(result.bubble_skin_id == "bubble_skin_gold", "valid bubble skin should be preserved", prefix) and ok
	ok = TestAssert.is_true(result.changed_fields.is_empty(), "valid loadout should not mark changed fields", prefix) and ok
	return ok
...
func _test_invalid_profile_values_fall_back_to_owned_catalog_assets() -> bool:
	...
	var prefix := "loadout_normalizer_test"
	var ok := true
	ok = TestAssert.is_true(result.character_id == "char_huoying", "invalid character should use owned catalog character", prefix) and ok
	ok = TestAssert.is_true(result.character_skin_id == "skin_gold", "invalid character skin should use owned catalog skin", prefix) and ok
	ok = TestAssert.is_true(result.bubble_style_id == "bubble_round", "invalid bubble should use owned catalog bubble", prefix) and ok
	ok = TestAssert.is_true(result.bubble_skin_id == "bubble_skin_gold", "invalid bubble skin should use owned catalog skin", prefix) and ok
	ok = TestAssert.is_true(result.changed_fields.size() == 4, "all invalid fields should be marked changed", prefix) and ok
	return ok
...
func _test_apply_to_ticket_request_uses_normalized_values() -> bool:
	...
	var prefix := "loadout_normalizer_test"
	var ok := true
	ok = TestAssert.is_true(request.selected_character_id == "char_huoying", "ticket should receive normalized character", prefix) and ok
	ok = TestAssert.is_true(request.selected_bubble_style_id == "bubble_round", "ticket should receive normalized bubble", prefix) and ok
	return ok
