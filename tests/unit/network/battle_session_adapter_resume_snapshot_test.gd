...
func _test_apply_resume_snapshot_pending_before_runtime_active() -> bool:
	...
	var ok := true
	ok = qqt_check(runtime.injected_messages.is_empty(), "inactive runtime should not inject immediately", prefix) and ok
	ok = qqt_check(adapter.pending_resume_snapshot == snapshot, "inactive runtime should keep pending resume snapshot", prefix) and ok
...
func _test_apply_resume_snapshot_injects_immediately_when_runtime_active() -> bool:
	...
	var prefix := "battle_session_adapter_resume_snapshot_test"
	var ok := true
	ok = qqt_check(runtime.injected_messages.size() == 1, "active runtime should inject resume checkpoint immediately", prefix) and ok
	if runtime.injected_messages.size() == 1:
		ok = qqt_check(int(runtime.injected_messages[0].get("tick", 0)) == 88, "injected checkpoint should match resume snapshot", prefix) and ok
	ok = qqt_check(adapter.pending_resume_snapshot == null, "injected resume snapshot should be cleared", prefix) and ok
