...
func _create_room_message(peer_id: int, room_kind: String, room_display_name: String) -> Dictionary:
	...
	var default_bubble_id := BubbleCatalogScript.get_default_bubble_id()
	var ticket := _make_room_ticket(
		{
			...
			"allowed_character_ids": [default_character_id],
			"allowed_character_skin_ids": [""],
			"allowed_bubble_style_ids": [default_bubble_id],
...
func _resume_room_message(peer_id: int, room_id: String, member_session: Dictionary, match_id: String) -> Dictionary:
	var ticket_room_id := String(member_session.get("room_id", room_id))
	var ticket := _make_room_ticket(
		{
			"ticket_id": "ticket_resume_%d" % peer_id,
...
func _make_room_ticket(claim_seed: Dictionary) -> Dictionary:
	var payload := claim_seed.duplicate(true)
	var json_text := JSON.stringify(payload)
	var encoded_payload := _to_base64_url(json_text.to_utf8_buffer())
	var signature := _sign_room_ticket(encoded_payload)
	return {
...
func _sign_room_ticket(encoded_payload: String) -> String:
