...
const Phase3DebugToolsScript = preload("res://app/flow/phase3_debug_tools.gd")
...
func _test_room_scene_uses_canonical_controller_contract() -> void:
	...
	file.close()
	_assert_true(text.contains("[ext_resource type=\"Script\" path=\"%s\"" % ROOM_SCENE_CONTROLLER_PATH), "room scene binds canonical controller ext resource")
...
func _test_reset_local_loop_ready_only_applies_when_debug_enabled() -> void:
	...
	var snapshot_on: RoomSnapshot = room_controller.build_room_snapshot()
	var local_on: RoomMemberState = _find_member(snapshot_on, 1)
	var remote_on: RoomMemberState = _find_member(snapshot_on, 2)
