...
const TestAssert = preload("res://tests/phase2/helpers/test_assert.gd")
...
func _test_encode_decode_roundtrip() -> bool:
	...
	ok = TestAssert.is_true(String(decoded.get("match_id", "")) == "phase4_codec_match", "match_id should survive roundtrip", prefix) and ok
	return ok
