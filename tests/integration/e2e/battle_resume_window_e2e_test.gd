...
func _ready() -> void:
	...
	ok = TestAssert.is_true(not token.is_empty(), "resume token hash source should exist for member", prefix) and ok

	coordinator.on_member_disconnected(member_id)
	var in_window := coordinator.try_resume(member_id, token, 9, "match_456")
	ok = TestAssert.is_true(bool(in_window.get("ok", false)), "resume in active window should succeed", prefix) and ok
	...
	ok = TestAssert.is_true(not bool(out_window.get("ok", true)) and String(out_window.get("error", "")) == "RESUME_WINDOW_EXPIRED", "resume out of window should fail", prefix) and ok
	...
	ok = TestAssert.is_true(not bool(after_logout.get("ok", true)) and String(after_logout.get("error", "")) == "MATCH_NOT_ACTIVE", "resume should fail after logout/inactive", prefix) and ok

	var token_hash := ResumeTokenUtilsScript.hash_resume_token(token)
	ok = TestAssert.is_true(not token_hash.is_empty(), "resume token hash generation should be valid", prefix) and ok

	if ok:
		print("battle_resume_window_e2e_test: PASS")
	else:
		push_error("battle_resume_window_e2e_test: FAIL")
