...
func _test_resume_window_timeout_triggers_match_finished_abort() -> bool:
	...
	if runtime == null:
		return qqt_check(true, "skip: ServerRoomRuntime removed after phase26", "active_match_resume_timeout_abort_test")
	...
	ok = qqt_check(binding != null, "disconnected client binding should remain available", prefix) and ok
	if binding != null:
	...
	ok = qqt_check(not finish_message.is_empty(), "expired resume window should broadcast MATCH_FINISHED", prefix) and ok
	if not finish_message.is_empty():
		var result: Dictionary = finish_message.get("result", {})
		ok = qqt_check(String(result.get("finish_reason", "")) == "peer_resume_timeout", "finish reason should be peer_resume_timeout", prefix) and ok
		ok = qqt_check(String(finish_message.get("resume_timeout_member_id", "")) == String(fixture["client_member_id"]), "finished message should include timed out member id", prefix) and ok
	ok = qqt_check(not runtime.is_match_active(), "resume timeout should stop active match", prefix) and ok
	ok = qqt_check(not bool(runtime._room_service.room_state.match_active), "room state should leave active match after timeout", prefix) and ok
...
func _test_poll_expired_skips_unexpired_members() -> bool:
	...
	ok = qqt_check(_latest_match_finished(broadcasts).is_empty(), "unexpired resume window should not finish match", prefix) and ok
	ok = qqt_check(runtime.is_match_active(), "unexpired resume window should keep match active", prefix) and ok
