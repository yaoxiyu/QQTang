...
func _main_body() -> void:
	...
	var ok := true
	var prefix := "client_room_runtime_ws_proto_join_room_test"
	ok = qqt_check(ws.sent_messages.size() == 1, "join_room should be forwarded to ws client", prefix) and ok
	if ws.sent_messages.size() > 0:
		ok = qqt_check(
			String(ws.sent_messages[0].get("message_type", "")) == TransportMessageTypesScript.ROOM_JOIN_REQUEST,
			"join_room should keep ROOM_JOIN_REQUEST semantics",
			prefix
		) and ok
	runtime.queue_free()
