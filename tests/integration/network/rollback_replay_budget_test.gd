...
func test_large_authority_gap_uses_force_resync_instead_of_single_frame_replay() -> void:
	...
	var predicted_world := _build_world(1357)
	...
	var authority_world := _build_world(1357)
	for tick_id in range(1, 41):
		_enqueue_idle_tick(authority_world, tick_id)
		authority_world.step()
	var authoritative_snapshot := snapshot_service.build_light_snapshot(authority_world, 40)

	rollback.set_predicted_until_tick(100)
	...
	assert_eq(rollback.predicted_until_tick, 40)
