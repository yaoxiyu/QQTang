...
func _main_body() -> void:
	...
	var ok := true
	var prefix := "client_room_runtime_create_room_canonical_message_test"
	ok = qqt_check(ws.sent_messages.size() == 1, "create_room should be sent through ws client", prefix) and ok
	if ws.sent_messages.size() > 0:
		ok = qqt_check(
			String(ws.sent_messages[0].get("message_type", "")) == TransportMessageTypesScript.ROOM_CREATE_REQUEST,
			"create_room message_type should keep canonical request semantics",
			prefix
		) and ok
	runtime.queue_free()
