...
func _test_two_rooms_keep_independent_runtime_and_match_state() -> bool:
	...
	registry.route_message(_create_room_message(1, "public_room", "Alpha", map_id, rule_id, mode_id))
	...
	registry.route_message({"message_type": TransportMessageTypesScript.ROOM_TOGGLE_READY, "sender_peer_id": 1})
	registry.route_message({"message_type": TransportMessageTypesScript.ROOM_TOGGLE_READY, "sender_peer_id": 2})
	registry.route_message({"message_type": TransportMessageTypesScript.ROOM_START_REQUEST, "sender_peer_id": 1})

	var public_room_id := String(registry.peer_room_bindings.get(1, ""))
	var private_room_id := String(registry.peer_room_bindings.get(3, ""))
