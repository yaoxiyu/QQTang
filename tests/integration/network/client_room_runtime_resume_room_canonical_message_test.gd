...
func _main_body() -> void:
	...
	var ok := true
	var prefix := "client_room_runtime_resume_room_canonical_message_test"
	ok = qqt_check(ws.sent_messages.size() == 1, "resume_room should be sent through ws client", prefix) and ok
	if ws.sent_messages.size() > 0:
		...
		ok = qqt_check(
			...
			"resume_room message_type should keep canonical request semantics",
			prefix
		) and ok
