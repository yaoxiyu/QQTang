...
func test_main() -> void:
	...
	var type2_run_a := _run_type2_footprint_case(20260501)
	var type2_run_b := _run_type2_footprint_case(20260501)
	...
	_assert(type2_run_a["checksums"] == type2_run_b["checksums"], "type2 footprint checksum sequence should match across identical runs")
	_assert(type2_run_a["covered_cells"] == type2_run_b["covered_cells"], "type2 footprint coverage should match across identical runs")
	_assert(type2_run_a["final_checksum"] == type2_run_b["final_checksum"], "type2 footprint final checksum should match across identical runs")
...
func _run_type2_footprint_case(seed: int) -> Dictionary:
	var world := SimWorld.new()
	world.rng = SimRng.new(seed)
	world.bootstrap(SimConfig.new(), {"grid": BuiltinMapFactory._build_from_rows([
		"##########",
	...
	world.config.system_flags["explosion_reaction"] = BattleExplosionConfigBuilder.new().build_for_rule("ruleset_classic")
	world.state.bubbles.spawn_bubble(1, 4, 4, 2, 1, 2, 2, 4)
	world.state.indexes.rebuild_from_state(world.state)

	var checksum_builder := ChecksumBuilder.new()
	var checksums: Array[int] = []
	var covered_cells: Array[String] = []
	for _step_index in range(2):
		var result := world.step()
		checksums.append(checksum_builder.build(world, world.state.match_state.tick))
		for event in result.get("events", []):
			if event != null and int(event.event_type) == SimEvent.EventType.BUBBLE_EXPLODED:
				covered_cells = _sort_cells(event.payload.get("covered_cells", []))
