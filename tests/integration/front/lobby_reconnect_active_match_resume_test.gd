...
func _test_lobby_reconnect_to_active_match_enters_resume_loading() -> bool:
	...
	var ok := true
	ok = TestAssert.is_true(app_runtime.current_loading_mode == "resume_match", "resume accepted should switch runtime loading mode", prefix) and ok
	ok = TestAssert.is_true(app_runtime.current_resume_snapshot == snapshot, "resume snapshot should be stored on runtime", prefix) and ok
	ok = TestAssert.is_true(app_runtime.current_start_config != null, "resume start config should be stored on runtime", prefix) and ok
	if app_runtime.current_start_config != null:
		ok = TestAssert.is_true(int(app_runtime.current_start_config.local_peer_id) == 9, "resume local peer should be reconnect transport peer", prefix) and ok
		ok = TestAssert.is_true(int(app_runtime.current_start_config.controlled_peer_id) == 3, "resume controlled peer should be original battle peer", prefix) and ok
