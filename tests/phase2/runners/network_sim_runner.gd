...
const FakeTransportScript = preload("res://tests/phase2/helpers/fake_transport.gd")
const DualClientRunnerScript = preload("res://tests/phase2/runners/dual_client_runner.gd")

var transport: FakeTransport = FakeTransportScript.new()
var dual_runner: DualClientRunner = null
...
func setup(config: Dictionary = {}) -> void:
	...
	dual_runner.setup(
		...
		config.get("map_data", TestMapFactory.build_basic_map())
	)
...
func run(input_by_tick: Array[Dictionary], flush_ticks: int = 8) -> Dictionary:
	...
	var server_tick := dual_runner.server.active_match.sim_world.state.match_state.tick
...
func _send_client_inputs(current_tick: int, tick_inputs: Dictionary) -> void:
	var peer_a := dual_runner.client_a.local_peer_id
	var peer_b := dual_runner.client_b.local_peer_id
...
func _queue_final_checkpoint(current_tick: int) -> void:
	var final_tick := dual_runner.server.active_match.sim_world.state.match_state.tick
