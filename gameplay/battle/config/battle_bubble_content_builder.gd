...
func build_for_start_config(start_config: BattleStartConfig) -> Array[Dictionary]:
	...
	for loadout in start_config.player_bubble_loadouts:
		...
		var entry := {
			...
			"type": int(metadata.get("type", 1)),
			"power": int(metadata.get("power", 1)),
			"footprint_cells": int(metadata.get("footprint_cells", 1)),
			"player_obtainable": bool(metadata.get("player_obtainable", true)),
