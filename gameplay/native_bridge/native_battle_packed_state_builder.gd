...
func append_bubble(state: Dictionary, bubble: Dictionary) -> void:
	...
	bubbles[base + Schema.BUBBLE_TYPE] = int(bubble.get("bubble_type", bubble.get("type", 0)))
	bubbles[base + Schema.BUBBLE_FOOTPRINT_CELLS] = int(bubble.get("footprint_cells", 1))
