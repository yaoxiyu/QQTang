...
func _ensure_runtime_message_router() -> void:
	...
	_runtime_message_router = _battle_session_bootstrap.ensure_runtime_message_router({
		...
		TransportMessageTypesScript.MATCH_RESUME_ACCEPTED: Callable(_network_gateway, "on_bootstrap_match_resume_accepted"),
		TransportMessageTypesScript.MATCH_RESUME_REJECTED: Callable(_network_gateway, "on_bootstrap_match_resume_rejected"),
		TransportMessageTypesScript.MATCH_LOADING_SNAPSHOT: Callable(_network_gateway, "on_bootstrap_match_loading_snapshot_message"),
