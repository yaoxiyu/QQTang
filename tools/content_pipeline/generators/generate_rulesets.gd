...
func generate() -> void:
    ...
    for i in range(1, lines.size()):
        ...
        def.drop_battle_backpack_on_death = get_cell(row, header_index, "drop_battle_backpack_on_death") == "true"
        ...
        def.trapped_timeout_sec = int(get_cell(row, header_index, "trapped_timeout_sec"))
